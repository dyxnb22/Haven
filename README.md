...
Inside the TUI, `/sessions`, `/fork RUN_ID`, `/diff`, and `@path` file
mentions steer the session; typing while a run is active queues the input for
the next turn rather than interrupting it.
...
| `docs/adr/` | 23 architecture decision records, from scope and the tool boundary through the OS sandbox, sessions, reasoning replay, the multi-file patch transaction, session runtime, headless write, and the data-driven architecture verdict (ADR 0023) |
