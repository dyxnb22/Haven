...
Reviewed for the learning baseline: 2026-08-14. The original raw scan (48k
files / 4.2M LOC,
...
| ~~low~~ RESOLVED | Export redaction masked only values of currently-set env secrets | `src/haven/interfaces/export.py` | — | Fixed 2026-08-13: a second pass masks well-known credential shapes (sk-, AKIA, ghp_, xox, AIza, SG., PEM). Shape-based on purpose — an entropy heuristic would eat digests and diffs |
...
| Recipes run arbitrary user-registered argv | A08-ish / by design | `.haven.toml [recipes]`, ADR 0013 | User authorizes once; fixed argv; supported backends wrap it; model can never supply a command | Locally-trusted-repo assumption, especially when no backend exists, stated in SECURITY.md |
...
the extreme-pressure context clamp, native-sandbox rather than VM isolation,
and measured char-based token budgeting. Any future change should reopen the
